...
import { type SubmitHandler, useForm, useFieldArray, type Control, useWatch } from 'react-hook-form'
import { api } from '~/utils/api'
import { toast } from 'react-hot-toast'
import { zodResolver } from '@hookform/resolvers/zod'
import { registrationInput, type GuestSchema, type RegistrationSchema } from '~/types'
...
const Form: NextPage = () => {
  const {
    ...
    defaultValues: {
      email: '',
      amount: 0,
      guests: [{ firstName: '', lastName: '', isChild: false, ticketType: 'both' }],
    },
  ...
  const { mutate: createRegistration, isLoading: isRegistering } =
    api.registrations.create.useMutation({
      onSuccess: () => {
        toast.success('You are now registered please check your inbox for a confirmation email')
        reset()
      },
      onError: () => {
        toast.error(
  ...
  const Total = ({ control }: { control: Control<RegistrationSchema> }) => {
    ...
    return <div className="w-full text-end text-xl font-bold">Total: ${total}</div>
  }

  return (
    <form
      className="w-3/4 rounded-2xl bg-zinc-700 p-12 shadow-md"
      onSubmit={handleSubmit(submitHandler)}
    >
      <div className="mb-4 flex flex-col">
        <label htmlFor="email" className="mb-1 block pr-4 font-bold md:mb-0 md:text-left">
          Email
        </label>
        <input
          type="email"
          ...
          className="w-full appearance-none rounded-lg border-2 border-zinc-800 bg-zinc-700 px-4 py-2 leading-tight focus:border-orange-takaro focus:bg-zinc-600
           focus:outline-none"
        />
      </div>
      <div>
            ...
            <div key={field.id}>
              ...
              <div className={`mt-6 ${index ? '' : 'hidden'}`}>
                ...
                <div className="mb-5 flex justify-between md:text-left">
                  <div className="my-5 flex-1 self-center text-xl font-bold">
                    {wordIndex + ' guest'}
                  </div>
                  {/* <label
                  ...
                  >
                    ...
                    />
                    <span className="m-0 rounded-l-lg bg-orange-takaro px-4 py-2 peer-checked:bg-zinc-900 peer-checked:text-slate-200 text-slate-800">
                      Adult
                    </span>
                    <span className="m-0 rounded-r-lg bg-zinc-900 px-4 py-2 peer-checked:bg-orange-takaro peer-checked:text-slate-200">
                      Child
                    </span>
                  </label> */}
                  <button
                    ...
                    className="focus:shadow-outline rounded-lg bg-zinc-900 my-2 px-4 py-2 font-bold shadow hover:bg-red-800 focus:outline-none"
                  >
                    Remove
                  </button>
                </div>
              ...
              <div className="mb-4 flex flex-col">
                <label
                  ...
                  className="mb-1 block pr-4 font-bold md:mb-0 md:text-left"
                >
                  First Name
                </label>
                <input
              ...
              {/* <div className="mb-2 block font-bold md:mb-0 md:text-left">
              ...
              <div className="mb-5 grid w-full gap-4 font-bold md:grid-cols-3">
                ...
                <label
                  ...
                  className={`inline-flex w-full cursor-pointer flex-col place-content-center rounded-lg border border-zinc-800  p-2 ${
                    watchChild ? "text-zinc-200" : "text-slate-200"
                  } hover:bg-zinc-600 peer-checked/both:bg-orange-takaro peer-checked/both:text-slate-800`}
                >
                ...
                <label
                  ...
                  className={`inline-flex w-full cursor-pointer flex-col place-content-center rounded-lg border border-zinc-800  p-2 text-slate-200 hover:bg-zinc-600 peer-checked/friday:bg-orange-takaro peer-checked/friday:text-slate-800`}
                >
                ...
                <label
                  ...
                  className={`inline-flex w-full cursor-pointer flex-col place-content-center rounded-lg border border-zinc-800  p-2 ${
                    watchChild ? "text-zinc-200" : "text-slate-200"
                  } hover:bg-zinc-600 peer-checked/saturday:bg-orange-takaro peer-checked/saturday:text-slate-800`}
                >
